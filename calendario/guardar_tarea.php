...
// 'c+' para abrir para lectura y escritura; si no existe, lo crea.
$fileHandle = fopen($filepath, 'c+');
if (!$fileHandle) {
    echo json_encode(['success' => false, 'message' => 'No se pudo abrir el archivo de tareas. Asegúrate de que los permisos de escritura sean correctos.']);
    exit;
}
...
try {
    // Si el archivo está vacío, inicializarlo como un objeto JSON vacío
    // Esto es importante para evitar errores al decodificar un archivo vacío.
    if (filesize($filepath) === 0) {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            // Si el JSON existente está corrupto, notificar y, opcionalmente, inicializar
            error_log("Error al decodificar tareas.json existente: " . json_last_error_msg());
            // Podrías decidir qué hacer aquí: sobreescribir (como en el ejemplo anterior)
            // o devolver un error si la corrupción es un problema crítico.
            // Para este caso, si llega JSON desde el cliente, lo sobrescribimos.
        }
    }
