...
// Contiene las funciones de renderizado del calendario.
// Depende de las variables globales (fechaActual, tareas, profesionales, vistaActual, etc.)
// que deben ser pasadas como argumentos o accesibles desde script.js.
...
function generarHoras() {
    console.log("DEBUG: [generarHoras] Entrando en generarHoras()...");
    console.log(`DEBUG: [generarHoras] HORA_INICIO_JORNADA: ${HORA_INICIO_JORNADA}`);
    console.log(`DEBUG: [generarHoras] HORA_FIN_JORNADA: ${HORA_FIN_JORNADA}`);
    console.log(`DEBUG: [generarHoras] INTERVALO_MINUTOS: ${INTERVALO_MINUTOS}`);
    ...
    console.log("DEBUG: [generarHoras] Selectores encontrados:", horaSelects);

    let selectoresEncontrados = false; // Flag para saber si al menos un selector fue encontrado
    ...
    horaSelects.forEach((select, index) => {
        if (select) { // Asegurarse de que el elemento existe
            ...
            console.log(`DEBUG: [generarHoras] Selector ${index} (ID: ${select.id}) encontrado y primera opción añadida.`);
            selectoresEncontrados = true;
        } else {
            console.warn(`DEBUG: [generarHoras] Selector ${index} (esperado ID: ${['hora', 'editar-hora'][index]}) NO encontrado en el DOM.`);
        }
    });

    if (!selectoresEncontrados) {
        console.error("DEBUG: [generarHoras] Ninguno de los selectores de hora fue encontrado. Verifica tus IDs en el HTML.");
        return; // Salir si no hay selectores para llenar
    }

    if (HORA_INICIO_JORNADA >= HORA_FIN_JORNADA || INTERVALO_MINUTOS <= 0) {
        console.error("DEBUG: [generarHoras] Las constantes de tiempo son inválidas. Revisa HORA_INICIO_JORNADA, HORA_FIN_JORNADA, INTERVALO_MINUTOS.");
    ...
    for (let h = HORA_INICIO_JORNADA; h < HORA_FIN_JORNADA; h++) {
        for (let m = 0; m < 60; m += INTERVALO_MINUTOS) {
            ...
            // console.log(`DEBUG: [generarHoras] Añadiendo hora: ${tiempo}`); // Descomentar para ver cada hora generada
    ...
    console.log("DEBUG: [generarHoras] Finalizado generarHoras(). Comprueba los desplegables.");
}
...
function seleccionarDia(dia, tareas, profesionales, filtroProfesional) {
    console.log("DEBUG: Entrando en seleccionarDia para fecha:", dia);
    const fechaSeleccionadaStr = formatearFecha(dia); // Usa tu función formatearFecha
    console.log("DEBUG: Fecha formateada para selección:", fechaSeleccionadaStr);

    // Deseleccionar el día previamente seleccionado si existe
    if (diaSeleccionadoElement) {
        diaSeleccionadoElement.classList.remove('dia-seleccionado');
    }
    // Seleccionar el nuevo día
    const nuevoDiaElement = document.querySelector(`.dia[data-date="${fechaSeleccionadaStr}"]`);
    
    if (nuevoDiaElement) {
        nuevoDiaElement.classList.add('dia-seleccionado');
        diaSeleccionadoElement = nuevoDiaElement; // Actualiza la referencia al día seleccionado
        console.log("DEBUG: Día seleccionado en el calendario:", fechaSeleccionadaStr);
        window.mostrarTareasDelDia(fechaSeleccionadaStr, tareas, profesionales, filtroProfesional); // Asumo que esta función existe en script.js
    } else {
        console.warn(`DEBUG WARNING: No se encontró el elemento .dia con data-date="${fechaSeleccionadaStr}"`);
        // Si no se encuentra el día, asegúrate de limpiar el panel de tareas o mostrar un mensaje apropiado
        const listaTareasDia = document.getElementById("lista-tareas-dia");
        if (listaTareasDia) {
            listaTareasDia.innerHTML = '<li>No se pudo seleccionar el día en el calendario.</li>';
        }
    }
    return nuevoDiaElement; // Devuelve el elemento seleccionado
}
...
window.renderizarTareasDelDiaSeleccionado = renderizarTareasDelDiaSeleccionado;
// Asegúrate de que estas funciones se llamen en el momento adecuado en script.js   